	...
	<div id = "main">
		...
		<h2>支払い方法を選択してください</h2>
			...
			<s:form action= "PaymentConfirmAction">
				<span class= "error"><s:if test="blankCard != null">
					<s:property value="blankCard" escape="false" />
				</s:if></span></s:form>
		</div>


			<tr><th>ユーザーID</th><td><s:property value= "#session.paymentUserInfoDTO.userId" />&thinsp;&thinsp;</td></tr>
			<tr><td class="bar" colspan="6"></td> </tr>

			<tr><th>氏名</th><td><s:property value="familyName"/>&thinsp;&thinsp;<s:property value="#session.paymentUserInfoDTO.firstName"/></td></tr>
			<tr><td class="bar" colspan="6"></td> </tr>

			<tr><th>ふりがな</th><td><s:property value="familyNameKana"/>&thinsp;&thinsp;&thinsp;<s:property value="#session.paymentUserInfoDTO.firstNameKana"/></td></tr>
			<tr><td class="bar" colspan="6"></td> </tr>

			<tr><th>性別</th><td><s:if test="#session.paymentUserInfoDTO.sex == 0">男性</s:if><s:if test="#session.paymentUserInfoDTO.sex == 1">女性</s:if></td></tr>
			<tr><td class="bar" colspan="6"></td> </tr>

			<tr><th>メールアドレス</th><td><s:property value="#session.paymentUserInfoDTO.email" /></td></tr>
			<tr><td class="bar" colspan="6"></td> </tr>

			<s:if test= '<s:property value="#session.paymentUserInfoDTO.telNumber" /> '><tr><th>電話番号</th><td><s:property value="#session.paymentUserInfoDTO.telNumber" /></td></tr>
			...
			<tr>
				<s:if test='cardExist == null'>
					<th>クレジットカード</th><td>登録されていません。</td>
					<td><input id="page" type="button" onclick="location.href=' <s:url action= "CardCreateAction" ><s:param name="fromPayment" value="1" /></s:url>'" value="クレジットカード新規" /></td>
				</s:if>
				<s:if test='cardExist == "1"'>
					<th>クレジットカード（番号）</th><td><s:iterator value="cardUpdateDTO"><s:property value="cardNumber"/></s:iterator></td>
					<td><input id="page" type="button" onclick="location.href=' <s:url action= "CardUpdateAction" ><s:param name="fromPayment" value="1" /></s:url>'"value="クレジットカード編集"></td>
				</s:if>
			</tr>
			<tr><td class="bar" colspan="6"></td> </tr>
			...
			<tr><th>支払い方法</th><td>
				...
				<s:submit value="購入確認へ" id="btnConfirmation" class= "submit" /></td></tr>
			</s:form>
						<tr><td class="bar" colspan="6"></td> </tr>

			<tr><td><a id= "page" href='<s:url action="GoHomeAction" />' >Homeへ戻る</a></td><td>

<a id="page" href='<s:url action="PaymentAction" />' >前のページへ戻る</a></td></tr>
		</table>
	</div>
